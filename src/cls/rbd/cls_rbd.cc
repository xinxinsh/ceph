...
int get_old_snap_collections(cls_method_context_t hctx, bufferlist &bl, struct rbd_obj_header_ondisk *header,
			  const char *snap_names) {
   int r = snap_read_header(hctx, bl);
   if (r < 0)
     return r;
   header = (struct rbd_obj_header_ondisk *)bl.c_str();
   int snaps_id_ofs = sizeof(*header);
   int names_ofs = snaps_id_ofs + sizeof(struct rbd_obj_header_ondisk) * header->snap_count;
   snap_names = ((char *)header) + names_ofs;
   return 0;
}
...
{
  bufferlist bl;
  struct rbd_obj_header_ondisk *header = NULL;
  char *name = NULL;
  int rc = get_old_snap_collections(hctx, bl, header, name);
  if (rc < 0)
    return rc;
...
int old_snapshot_add(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  const char *snap_names = NULL;
  int rc = get_old_snap_collections(hctx, bl, header, snap_names);
  if (rc < 0)
    return rc;
