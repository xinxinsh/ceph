...
#undef dout_prefix
#define dout_prefix *_dout << "genericcstorebackend(" << get_basedir_path() << ") "

#define ALIGN_DOWN(x, by) ((x) - ((x) % (by)))
#define ALIGNED(x, by) (!((x) % (by)))
#define ALIGN_UP(x, by) (ALIGNED((x), (by)) ? (x) : (ALIGN_DOWN((x), (by)) + (by)))

GenericCStoreBackend::GenericCStoreBackend(CStore *fs):
  CStoreBackend(fs),
  ioctl_fiemap(false),
...
int GenericCStoreBackend::detect_features()
{
  char fn[PATH_MAX];
...
int GenericCStoreBackend::create_current()
{
  struct stat st;
...
int GenericCStoreBackend::syncfs()
{
  int ret;
  if (m_filestore_fsync_flushes_journal_data) {
...
int GenericCStoreBackend::do_fiemap(int fd, off_t start, size_t len, struct fiemap **pfiemap)
{
...
int GenericCStoreBackend::_crc_load_or_init(int fd, SloppyCRCMap *cm)
{
  char buf[100];
  bufferptr bp;
  int r = 0;
...
int GenericCStoreBackend::_crc_save(int fd, SloppyCRCMap *cm)
{
  bufferlist bl;
  ::encode(*cm, bl);
...
int GenericCStoreBackend::_crc_update_write(int fd, loff_t off, size_t len, const bufferlist& bl)
{
...
int GenericCStoreBackend::_crc_update_truncate(int fd, loff_t off)
{
  SloppyCRCMap scm(get_crc_block_size());
  int r = _crc_load_or_init(fd, &scm);
  if (r < 0)
...
int GenericCStoreBackend::_crc_update_zero(int fd, loff_t off, size_t len)
{
  SloppyCRCMap scm(get_crc_block_size());
  int r = _crc_load_or_init(fd, &scm);
  if (r < 0)
...
int GenericCStoreBackend::_crc_update_clone_range(int srcfd, int destfd,
...
int GenericCStoreBackend::_crc_verify_read(int fd, loff_t off, size_t len, const bufferlist& bl,
					      ostream *out)
{
  SloppyCRCMap scm(get_crc_block_size());
  int r = _crc_load_or_init(fd, &scm);
