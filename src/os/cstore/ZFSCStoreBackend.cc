...
#undef dout_prefix
#define dout_prefix *_dout << "zfscstorebackend(" << get_basedir_path() << ") "

ZFSCStoreBackend::ZFSCStoreBackend(CStore *fs) :
  GenericCStoreBackend(fs), base_zh(NULL), current_zh(NULL),
  m_filestore_zfs_snap(g_conf->filestore_zfs_snap)
{
  int ret = zfs.init();
  if (ret < 0) {
    dout(0) << "ZFSCStoreBackend: failed to init libzfs" << dendl;
    return;
  }

  base_zh = zfs.path_to_zhandle(get_basedir_path().c_str(), ZFS::TYPE_FILESYSTEM);
  if (!base_zh) {
    dout(0) << "ZFSCStoreBackend: failed to get zfs handler for basedir" << dendl;
    return;
  }
...
ZFSCStoreBackend::~ZFSCStoreBackend()
{
  if (base_zh)
    zfs.close(base_zh);
  if (current_zh)
...
int ZFSCStoreBackend::update_current_zh()
{
  char path[PATH_MAX];
...
int ZFSCStoreBackend::detect_features()
{
  if (!current_zh)
...
bool ZFSCStoreBackend::can_checkpoint()
{
  return m_filestore_zfs_snap && current_zh != NULL;
}

int ZFSCStoreBackend::create_current()
{
  struct stat st;
...
int ZFSCStoreBackend::list_checkpoints(list<string>& ls)
{
...
int ZFSCStoreBackend::create_checkpoint(const string& name, uint64_t *cid)
{
...
int ZFSCStoreBackend::rollback_to(const string& name)
{
...
int ZFSCStoreBackend::destroy_checkpoint(const string& name)
{
