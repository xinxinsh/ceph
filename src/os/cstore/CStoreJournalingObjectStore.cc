...
void CStoreJournalingObjectStore::journal_start()
{
  dout(10) << "journal_start" << dendl;
  finisher.start();
}

void CStoreJournalingObjectStore::journal_stop()
{
...
void CStoreJournalingObjectStore::journal_write_close()
{
  if (journal) {
    journal->close();
    delete journal;
...
int CStoreJournalingObjectStore::journal_replay(uint64_t fs_op_seq)
{
...
uint64_t CStoreJournalingObjectStore::ApplyManager::op_apply_start(uint64_t op)
{
...
void CStoreJournalingObjectStore::ApplyManager::op_apply_finish(uint64_t op)
{
  Mutex::Locker l(apply_lock);
...
uint64_t CStoreJournalingObjectStore::SubmitManager::op_submit_start()
{
  lock.Lock();
  uint64_t op = ++op_seq;
...
void CStoreJournalingObjectStore::SubmitManager::op_submit_finish(uint64_t op)
{
...
void CStoreJournalingObjectStore::ApplyManager::add_waiter(uint64_t op, Context *c)
{
...
bool CStoreJournalingObjectStore::ApplyManager::commit_start()
{
  bool ret = false;
...
void CStoreJournalingObjectStore::ApplyManager::commit_started()
{
  Mutex::Locker l(apply_lock);
...
void CStoreJournalingObjectStore::ApplyManager::commit_finish()
{
  Mutex::Locker l(com_lock);
...
void CStoreJournalingObjectStore::_op_journal_transactions(
