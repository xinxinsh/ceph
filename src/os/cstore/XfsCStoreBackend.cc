...
#undef dout_prefix
#define dout_prefix *_dout << "xfscstorebackend(" << get_basedir_path() << ") "

XfsCStoreBackend::XfsCStoreBackend(CStore *fs):
  GenericCStoreBackend(fs), m_has_extsize(false) { }
 ...
 */
int XfsCStoreBackend::set_extsize(int fd, unsigned int val)
{
  struct fsxattr fsx;
  struct stat sb;
  int ret;
...
int XfsCStoreBackend::detect_features()
{
  int ret;

  ret = GenericCStoreBackend::detect_features();
  if (ret < 0)
    return ret;
...
int XfsCStoreBackend::set_alloc_hint(int fd, uint64_t hint)
{
  if (!m_has_extsize)
    return -EOPNOTSUPP;
