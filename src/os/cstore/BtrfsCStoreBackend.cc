...
#undef dout_prefix
#define dout_prefix *_dout << "btrfscstorebackend(" << get_basedir_path() << ") "

#define ALIGN_DOWN(x, by) ((x) - ((x) % (by)))
#define ALIGNED(x, by) (!((x) % (by)))
#define ALIGN_UP(x, by) (ALIGNED((x), (by)) ? (x) : (ALIGN_DOWN((x), (by)) + (by)))

BtrfsCStoreBackend::BtrfsCStoreBackend(CStore *fs):
    GenericCStoreBackend(fs), has_clone_range(false),
...
int BtrfsCStoreBackend::detect_features()
{
  int r;

  r = GenericCStoreBackend::detect_features();
  if (r < 0)
    return r;
...
bool BtrfsCStoreBackend::can_checkpoint()
{
  return stable_commits;
}

int BtrfsCStoreBackend::create_current()
{
  struct stat st;
...
int BtrfsCStoreBackend::list_checkpoints(list<string>& ls)
{
  int ret, err = 0;
...
int BtrfsCStoreBackend::create_checkpoint(const string& name, uint64_t *transid)
{
...
int BtrfsCStoreBackend::sync_checkpoint(uint64_t transid)
{
  // wait for commit
...
int BtrfsCStoreBackend::rollback_to(const string& name)
{
...
int BtrfsCStoreBackend::destroy_checkpoint(const string& name)
{
...
int BtrfsCStoreBackend::clone_range(int from, int to, uint64_t srcoff, uint64_t len, uint64_t dstoff)
{
