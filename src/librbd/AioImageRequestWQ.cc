...
ssize_t AioImageRequestWQ::compare_and_write(uint64_t off, uint64_t len,
                                                bufferlist &&cmp_bl,
                                                bufferlist &&bl,
...
void AioImageRequestWQ::aio_compare_and_write(AioCompletion *c,
  ...
  RWLock::RLocker owner_locker(m_image_ctx.owner_lock);
  if (m_image_ctx.non_blocking_aio || writes_blocked()) {
		queue(new AioImageCompareAndWrite<>(m_image_ctx, c,
                                        off, len,
                                        std::move(cmp_bl),
    ...
    c->start_op();
    AioImageRequest<>::aio_compare_and_write(&m_image_ctx, c, off, len,
