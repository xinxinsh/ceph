...
#define READ_SIZE 4096
  ...
  int copy(ImageCtx *src, ImageCtx *dest, ProgressContext &prog_ctx)
  {
    ...
    for (uint64_t offset = 0; offset < src_size; offset += period) {
      ...
      AioCompletion *comp = AioCompletion::create_and_start(ctx, src,
                                                            AIO_TYPE_READ);
      AioImageRequest<>::aio_read(src, comp, {{offset, len}}, ReadResult{&bl},
                                  fadvise_flags);
		       ...
		       void *arg)
  {
    ...
    while (left > 0) {
      ...
      AioCompletion *c = AioCompletion::create_and_start(&ctx, ictx,
                                                         AIO_TYPE_READ);
      AioImageRequest<>::aio_read(ictx, c, {{off, read_len}}, ReadResult{&bl}, 0);
  ...
  int disk_usage(ImageCtx *ictx, uint64_t* used_size)
  {
    ldout(ictx->cct, 20) << "disk_usage " << ictx << dendl;
     
    std::vector<librbd::snap_info_t> snaps;
    std::vector<librbd::snap_info_t>::const_iterator snap;
    ...
    for (snap = snaps.begin(); snap != snaps.end(); ++snap) {
      used = 0;
      const char* from = NULL;
      if (!last_snap_name.empty())
        from = last_snap_name.c_str();

      snap_set(ictx, snap->name.c_str());
      r = diff_iterate(ictx, from, 0, snap->size, false, true,
    ...
    if (!last_snap_name.empty())
      from = last_snap_name.c_str();
    r = diff_iterate(ictx, from, 0, size, false, true,
                     disk_usage_cb, (void *)(&used));
    if (r < 0) {
