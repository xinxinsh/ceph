...
struct AioImageRequest<MockReplayImageCtx> {
  ...
  MOCK_METHOD6(aio_compare_and_write, void(AioCompletion *c, uint64_t off, uint64_t len,
  ...
  static void aio_compare_and_write(MockReplayImageCtx *ictx, AioCompletion *c,
    ...
    assert(s_instance != nullptr);
    s_instance->aio_compare_and_write(c, off, len, cmp_bl, bl,
...
public:
  ...
  void expect_aio_discard(MockAioImageRequest &mock_aio_image_request,
                          ...
                          uint64_t len, skip_partial_discard) {
    EXPECT_CALL(mock_aio_image_request, aio_discard(_, off, len, skip_partial_discard))
...
TEST_F(TestMockJournalReplay, AioDiscard) {
  ...
  C_SaferCond on_safe;
  expect_aio_discard(mock_aio_image_request, &aio_comp, 123, 456, ictx->skip_partial_discard);
