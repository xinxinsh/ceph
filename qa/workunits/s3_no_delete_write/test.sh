...
echo "starting s3_no_delete workunit"
...
create_file() {
	...
	run_s3 put $S3_TEST_BUCKET_NAME/$filename filename=$tmpdir/$filename
}

get_file() {
	filename=$1
	dest_fname=$2
	run_s3 get $S3_TEST_BUCKET_NAME/$filename filename=$tmpdir/$dest_fname
...
get_acl() {
	filename=$1
	dest_fname=$2
	run_s3 getacl $S3_TEST_BUCKET_NAME/$filename filename=$tmpdir/$dest_fname
}

set_acl() {
	filename=$1
	src_fname=$2
	run_s3 setacl $S3_TEST_BUCKET_NAME/$filename filename=$tmpdir/$src_fname
}

delete_file() {
	filename=$1
	run_s3 delete $S3_TEST_BUCKET_NAME/$filename
}
...
main() {
	...
	log echo "****************************************************************"
	init
	do_op 1 create_bucket $S3_TEST_BUCKET_NAME
	do_op 1 create_bucket $S3_TEST_BUCKET_NAME
	...
	get_anon 0 $S3_TEST_BUCKET_NAME foo foo.anon
